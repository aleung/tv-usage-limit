...
### A. Boot & Startup
1.  **Event**: TV Power On.
2.  **Action**: `BootReceiver` starts `TimeTrackingService`.
3.  **Feedback**: Small Toast message: *"TV Limit Active: Child Profile"*.
4.  **State**: Usage timer starts counting.

### B. Usage Warning (The "Limits")
...
2.  **UI**: **System Overlay** (High Priority Window).
    ...
    - Center Box:
        ...
        - Input: Numeric Keypad for PIN.
3.  **Interaction**:
    - **No Action**: At T=0, call `DevicePolicyManager.lockNow()` (Sleep).
    - **Input PIN**:
        - Limit is reset/overridden (switch profile).
        - Overlay dismisses.

### C. Admin / Settings UI
*Entry Point: Launch App from Launcher.*
1.  **Auth**: Require Admin PIN to enter.
2.  **Dashboard**:
...
- **Auto-Revert**: Always revert to Default Profile when screen turns off. (No configuration needed).
- **Boot Message**: Always shown ("Monitoring Active"). (No configuration needed).
